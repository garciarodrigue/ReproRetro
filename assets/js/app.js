...
function cargarInfo(e){
    ...
    if(e.target.classList.contains('playEstudiar')){
        ...
        titlePlay='Playlist para Relajar-Armonic';
        descripcionPlay='Escucha y Relajate';
        ...
        descripcionPlay='Playlist Corta Venas';
        ...
        titlePlay='Playlist para Rapers';
        descripcionPlay='Playlist solo Lirics';
        ...
        .then(function(data) {
            ...
            data.forEach(music => {
                ...
                <li class="music">
                    ...
                    play-music"><i class="far fa-play-circle"></i></a>
                    <h3>${music.artista}</h3> 
...
function reproducirMusica(e) {
    if(e.target.parentElement.classList.contains('play-music')){
        ...
        controles.innerHTML=`<a href="#" class="btn control atras"><i class="fas
        fa-backward"></i></a>
        <audio src="${urlM}" style="width: 50vw;" controls
        autoplay><input type="text" value="${urlM}" style="display:
        none;"></audio>
        <a href="#" class="btn control siguiente"><i 
    ...
    if (e.target.parentElement.classList.contains('siguiente')) {
        musicArray.forEach(limusic => {
            if(limusic.children[0].value===audioUrl){
                let siguienteMusica = limusic.nextElementSibling.children[0].
                value;

                let elementoParaReproducido=limusic.nextElementSibling.
                children[1];
        ...
        musicArray.forEach(limusic => {
            if(limusic.children[0].value===audioUrl){
                let musicaAtras = limusic.previousElementSibling.
                children[0].value;

                let elementoParaReproducido=limusic.
...
function siguienteAtras(musica,reproducida) {
    controles.innerHTML=`<a href="#" class="btn control atras"><i class="fas
        fa-backward"></i></a>
        <audio src="${musica}" style="width: 50vw;" controls
        autoplay><input type="text" value="${musica}" style="display:
        none;"></audio>
        <a href="#" class="btn control siguiente"><i 
